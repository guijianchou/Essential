...
function Get-WinEvent {
    ...
    if ($log -notin 'Security', 'System', 'Application', 'Setup') { throw 'Unexpected mock channel.' }
...
try {
    ...
    Test-Case 'The two databases have the same table schema and preserve legacy data' {
        ...
        Assert-True ($null -eq $assistant.GetLatestResultAsync().GetAwaiter().GetResult()) 'Assistant mode exposed extended history.'
    ...
    Test-Case 'Mocked collector emits complete bounded evidence, redacts secrets and excludes command lines' {
        ...
        $summary = & $collector -OutputPath $evidencePath -FromUtc '2026-01-01T00:00:00Z' -ToUtc '2026-01-02T00:00:00Z' | ConvertFrom-Json
    ...
    Test-Case 'Permission denial and truncation are recorded instead of reported as complete' {
        ...
        $denied = & $collector -OutputPath (Join-Path $testRoot 'denied.json') -FromUtc '2026-01-01T00:00:00Z' -ToUtc '2026-01-02T00:00:00Z' | ConvertFrom-Json
        ...
        $global:LsaModeTestScenario = 'normal'
        $limited = & $collector -OutputPath (Join-Path $testRoot 'limited.json') -FromUtc '2026-01-01T00:00:00Z' -ToUtc '2026-01-02T00:00:00Z' -MaxEventsPerChannel 1 | ConvertFrom-Json
    ...
    Test-Case 'Python publisher results roundtrip through the actual .NET storage and date queries' {
        ...
        $result = @{
            ...
            Metadata = @{ SchemaVersion = 1; Mode = 'assistant'; RunId = $evidence.RunId; Producer = 'claude'; AnalysisModel = 'synthetic-model';
    ...
    Test-Case 'The viewer detects committed WAL changes without polling file timestamps' {
        ...
        try {
            ...
            Assert-True ((Test-Path -LiteralPath ($fresh.DatabasePath + '-wal')) -and $fresh.GetAuditRecordCountAsync().GetAwaiter().GetResult() -eq 1) 'The first external WAL commit was not visible.'
        }
        finally {
            $cancel.Cancel()
    ...
    Test-Case 'The beta build has an asInvoker manifest and includes the external workflow' {
        ...
        Assert-True ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).ProductVersion -eq '0.3.4 beta1') 'Beta product version was not applied.'
