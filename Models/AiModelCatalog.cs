...
public static class AiModelCatalog
{
    ...
    public static int Rank(string? model)
    {
        for (int i = 0; i < SupportedModels.Count; i++)
            if (string.Equals(SupportedModels[i], model, StringComparison.OrdinalIgnoreCase)) return i;
        return -1;
    }
